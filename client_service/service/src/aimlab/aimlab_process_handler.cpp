...
private:
    ...
    void backfill();
    // Use to clear out dangling videos.
    void cleanup();
...
AimlabProcessHandlerInstance::AimlabProcessHandlerInstance(const process_watcher::process::Process& p, const service::database::DatabaseApi* db):
    ...
    _recorder = std::make_unique<service::recorder::GameRecorder>(_process, shared::filesystem::getSquadOvRecordFolder(), shared::EGame::Aimlab);
...
void AimlabProcessHandlerInstance::cleanup() {
    ...
    for (const auto& f : fs::directory_iterator(dir)) {
        if (!_db->isAimlabVideoAssociatedWithTask(f.path().string())) {
            fs::remove(f.path());
        }
    }
}
