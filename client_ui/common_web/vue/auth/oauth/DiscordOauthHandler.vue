<template>
    <div class="d-flex flex-column justify-center align-center full-parent-height full-width">
        <div class="text-h6">
            We're working on syncing your Discord account with SquadOV. This might take a few seconds...
        </div>
        ...
        <v-snackbar
            ...
            color="error"
        >
            Something went wrong when authenticating your Discord account. Please submit a bug report and try again!
        </v-snackbar>
    </div>
</template>
...
@Component
export default class DiscordOauthHandler extends Vue {
    @Prop({required: true})
    state!: string
    ...
    mounted() {
        apiClient.submitDiscordOauthAuthorization(this.code, this.state, window.location.href).then(() => {
            this.showSuccess = true
            this.startRedirectCountdown()
        }).catch((err: any) => {
            console.error('Failed to submit Discord authorization code: ', err)
            this.showError = true
        })
    }
}
