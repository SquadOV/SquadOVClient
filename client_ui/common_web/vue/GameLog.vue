<template>
  <v-container fluid class="full-parent-height">
    <div class="d-flex align-center">
      <v-menu
        offset-y
        :disabled="!hasSquadDropdown"
        v-model="showHideSquadMenu"
      >
        <template v-slot:activator="{ on, attrs }">
          <v-btn text v-bind="attrs" v-on="on">
            ...
            <v-icon small v-if="hasSquadDropdown"> mdi-chevron-down </v-icon>
          </v-btn>
        </template>
        ...
        <div dense v-if="!isLoadingAllSquads">
          <template v-for="(sq, sIdx) in allSquads">
            <v-menu offset-x :key="`squad-${sIdx}`">
              <template v-slot:activator="{ on, attrs }">
                <div
                  :class="`d-flex justify-space-between align-center pa-2 squad-div ${
                    sq.id === squadId ? 'selected-squad' : ''
                  }`"
                  v-bind="attrs"
                  v-on="on"
                >
      ...
      <template v-if="!isLoadingCurrentUser">
        <router-link :to="homePageTo">
          <v-tooltip bottom :open-delay="1000">
            <template v-slot:activator="{ on, attrs }">
              <div
                :class="`ml-2 pa-2 d-flex align-center justify-center home-game game-div ${
                  isOnHome ? 'selected-game' : ''
                }`"
                v-on="on"
                v-bind="attrs"
              >
                <v-icon large> mdi-all-inclusive </v-icon>
              </div>
            </template>
        ...
        <template v-for="(game, idx) in supportedGames">
          <div
            :class="`pa-2 game-div ${
              $route.path.startsWith(game.route.path) ? 'selected-game' : ''
            }`"
            :key="`game-${idx}`"
            v-if="!game.disabled"
          >
...
export default class GameLog extends Vue {
  ...
  perSquadMembers: { [idx: number]: SquadMembership[] | undefined } | null =
    null;
  ...
  get isLoadingCurrentUser(): boolean {
    return (
      !this.logUser ||
      (!this.logSquad && this.squadId !== undefined && !isNaN(this.squadId))
    );
  }
  ...
  refreshAllSquads() {
    ...
    apiClient
      .getUserSquads(this.$store.state.currentUser.id)
      .then((resp: ApiData<SquadMembership[]>) => {
  ...
  refreshSquad() {
    ...
    apiClient
      .getSquad(this.squadId)
      .then((resp: ApiData<Squad>) => {
